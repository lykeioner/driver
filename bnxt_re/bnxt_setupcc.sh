...
niccli_rem_app_tlv() {
	...
	if [ $j -eq 3 ]
	then
		del_commands+=("apptlv -del -app $APP_0")
		j=0
	fi
	done
	printf "%s\n" "${del_commands[@]}"
...
niccli_pgm_pfc_ets() {
	...
	if [ $CNP_SERVICE_TYPE -eq 1 ]; then
		...
		commands+=("ets -tsa 0:ets,1:ets,2:strict,3:strict,4:strict,5:strict,6:strict,7:strict -up2tc $pri2tc -tcbw $L2_BW,$ROCE_BW")
		commands+=("ets -tsa 0:ets,1:ets,2:strict -up2tc $pri2tc -tcbw $L2_BW,$ROCE_BW")
	else
		commands+=("ets -tsa 0:ets,1:ets -up2tc $pri2tc -tcbw $L2_BW,$ROCE_BW")
	fi

	if [ ! -z "$ROCE_PRI" ] && [ $ENABLE_PFC -eq 1 ]; then
		commands+=("pfc -enable $(printf "%d" $ROCE_PRI)")
		commands+=("apptlv -add -app $(printf "%d" $ROCE_PRI),3,4791")
	else
		commands+=("pfc -enable 0xFF")
	fi

	if [ $ENABLE_DSCP_BASED_PFC -eq 1 ] || [ $EN_ROCE_DSCP -eq 1 ]; then
		commands+=("apptlv -add -app $(printf "%d" $ROCE_PRI),5,$(printf "%d" $ROCE_DSCP)")
	fi

	if [ $ENABLE_CC -eq 1 ] && [ $CNP_SERVICE_TYPE -eq 1 ]; then
		commands+=("apptlv -add -app $(printf "%d" $ROCE_CNP_PRI),5,$(printf "%d" $ROCE_CNP_DSCP)")
	fi

	niccli -dev $PCI_BUSADDR <<EOF
$(printf "%s\n" "${commands[@]}")
EOF
}
